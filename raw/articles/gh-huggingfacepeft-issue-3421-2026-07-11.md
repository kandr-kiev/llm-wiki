...
**Fix**: register each expert under its own original key (the same one already used for `set_adapter` a few lines later) instead of `str(i)`. This matches how `XLoraLinearLayer`/`XLoraEmbeddingLayer`/`XLoraConv2dLayer.forward` already look up adapters -- by name, out of `self.target.lora_A`/`lora_embedding_A` -- and how `XLoraConfig`'s own docstring describes the `adapters` dict ("only the keys matter during loading"), so the user-facing key becomes the one identifier used everywhere, with no silent renaming in between.
...
- `test_delete_adapter`, reusing the existing 4-expert `model` fixture: calls `model.delete_adapter("1")`, then calls `.generate()` again.

Fail-before / pass-after, verified locally against the real (non-mocked) library:
- Before the fix (`git stash`-ed only `src/peft/tuners/xlora/model.py`, keeping the new tests in place): both new tests fail, with `KeyError: 'adapter_1'` and `KeyError: '1'` respectively, both raised from `disable_adapter_layers()` at `tuners_utils.py:521` -- exactly the mechanism described above.
- After restoring the fix (`git stash pop`): both tests pass.
- Ran the full `tests/test_xlora.py` file: 18 passed (16 pre-existing + 2 new), no regressions.
- `ruff check` and `ruff format --check` pass on both changed files.

## AI assistance disclosure

This PR was prepared with AI assistance (Claude Code). I independently verified both root causes by reading the current `xlora/model.py`, the relevant parts of `tuners_utils.py` (`BaseTuner.disable_adapter_layers`/`enable_adapter_layers`/`delete_adapter`/`set_adapter`, and the `BaseTunerLayer`-vs-`BaseTuner` split in how `active_adapter` is implemented), `XLoraLinearLayer`/`XLoraEmbeddingLayer`/`XLoraConv2dLayer.forward`, and `XLoraConfig`; reproduced both crashes end-to-end against the real, unmocked library before making any change; and reviewed every changed line before proposing this PR.
