...
> Regarding the proposed solution: I think it works but what we could do instead is to sum all the active `c3a_kernel`s and then send them through `BlockCircularConvolution.apply` instead of the other way round. This should get the same result and be more efficient. WDYT?
...
This PR was prepared with AI assistance (Claude Code). The bug and the requested fix approach were already documented publicly by @BenjaminBossan and @Chessing234 on #3164; I independently re-verified the bug still reproduces on current `main` (both failure modes, via a standalone script) before writing any code, implemented the approach @BenjaminBossan requested there, and verified the fix with the fail-before/pass-after testing described above. I reviewed every changed line and ran all tests referenced in this description myself.
