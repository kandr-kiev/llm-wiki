...
With USE_C10D_NCCL undefined, each guarded file preprocesses to an empty translation unit, so <nccl.h> is no longer reached and the backend compiles out cleanly for non-NCCL backends (e.g. libtorch_mtia_simt).
