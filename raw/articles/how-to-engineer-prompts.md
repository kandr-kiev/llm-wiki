...
type: playbook
description: Actionable runbook for systematic prompt engineering: structure, techniques, testing, and optimization
created: 2026-07-06
updated: 2026-07-06
tags: [playbook, prompt-engineering, reference, automation]
...
```
First, generate your answer. Then, review it for:
1. Accuracy — are all facts correct?
2. Completeness — did you address all parts?
3. Clarity — is it easy to understand?

If any issues found, revise and provide the improved version.
```
...
```
Here is a prompt I'm using: "{original_prompt}"

Analyze this prompt and suggest 3 improvements that would increase
the quality and reliability of the model's output. For each suggestion,
explain why it helps and provide the revised prompt.
```
...
| Problem | Cause | Fix |
|---|---|---|
| Model ignores instructions | Prompt too long/complex | Simplify, use bullet points |
| Inconsistent output | Temperature too high | Reduce to 0.1–0.3 |
| Hallucinations | No constraints/examples | Add examples, specify format |
| Too verbose | No length constraint | Add "be concise" or word limit |
| Wrong format | Output format not specified | Add JSON schema or template |
| Slow response | CoT adds tokens | Use CoT only for complex tasks |
