...
ingested: 2026-07-13
sha256: 76598afd21e6b94fa48618b2562a624e950eba6ce5723a02b79bd1ea8c23faf8
blog_source: ArXiv AI
---
<!DOCTYPE html>
<html lang="en">

<head><script>document.documentElement.classList.add('js');</script>  <title>[2607.09153] KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling</title>
...
<meta name="twitter:image:alt" content="arXiv logo"/>
  ...
  <script src="/static/browse/0.3.4/js/cite.js" type="text/javascript"></script><meta name="citation_title" content="KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling" /><meta name="citation_author" content="Kuang, Peng" /><meta name="citation_author" content="Jin, Haibo" /><meta name="citation_author" content="Han, Xiaoyu" /><meta name="citation_author" content="Wang, Yanli" /><meta name="citation_author" content="Yuan, Xiaopeng" /><meta name="citation_author" content="Yu, Ye" /><meta name="citation_author" content="Xu, Kaidi" /><meta name="citation_author" content="Wang, Haohan" /><meta name="citation_date" content="2026/07/10" /><meta name="citation_online_date" content="2026/07/10" /><meta name="citation_pdf_url" content="https://arxiv.org/pdf/2607.09153" /><meta name="citation_arxiv_id" content="2607.09153" /><meta name="citation_abstract" content="Process Reward Models (PRMs) have been proven to be highly effective in guiding test-time scaling (TTS) methods, which significantly boost the capabilities of LLM-based multi-agent systems. However, existing PRMs are text-based: they re-encode the entire trajectory text from scratch. In long multi-agent rollouts, the scoring cost, growing quadratically with respect to sequence length L, creates a severe computational bottleneck, severely limiting PRMs&#39; application in long-context scenarios. To resolve this, we introduce KV-PRM, a highly efficient process reward model that eliminates the heavy text re-encoding by directly reading the KV cache produced naturally during the LLM&#39;s generation phase. By processing a single &#34;verify token&#34; against the pre-existing KV cache, KV-PRM reduces the scoring cost from O(L^2) to O(L). We formally prove that the KV cache contains strictly greater information capacity than text, and is more efficient for downstream reward modeling. Empirically, across the MATH, GSM8K, and AIME benchmarks, KV-PRM matches or strictly outperforms text-PRMs under various TTS methods such as Beam Search, MCTS, and Weighted Voting, with up to a 5,000x reduction in scoring FLOPs, a 37x reduction in latency, and a 34x reduction in per-sequence memory footprint compared to text-based PRMs." />
</head>
...
rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
    ...
    <rdf:Description
        rdf:about="/abs/2607.09153"
        dc:identifier="/abs/2607.09153"
        dc:title="KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling"
        trackback:ping="/trackback/2607.09153" />
    </rdf:RDF>
--><div id="abs-outer">
  ...
  <div class="leftcolumn">
    ...
    <div class="header-breadcrumbs-mobile">
      <strong>arXiv:2607.09153</strong> (cs)
    </div>
...
<div id="content-inner">
  ...
  [Submitted on 10 Jul 2026]</div>
    <h1 class="title mathjax"><span class="descriptor">Title:</span>KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling</h1>
    <div class="authors"><span class="descriptor">Authors:</span><a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Kuang,+P" rel="nofollow">Peng Kuang</a>, <a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Jin,+H" rel="nofollow">Haibo Jin</a>, <a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Han,+X" rel="nofollow">Xiaoyu Han</a>, <a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Wang,+Y" rel="nofollow">Yanli Wang</a>, <a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Yuan,+X" rel="nofollow">Xiaopeng Yuan</a>, <a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Yu,+Y" rel="nofollow">Ye Yu</a>, <a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Xu,+K" rel="nofollow">Kaidi Xu</a>, <a href="https://arxiv.org/search/cs?searchtype=author&amp;query=Wang,+H" rel="nofollow">Haohan Wang</a></div>            <div id="download-button-info" hidden>View a PDF of the paper titled KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling, by Peng Kuang and 7 other authors</div>
    <a class="mobile-submission-download" href="/pdf/2607.09153">View PDF</a>
    <a class="mobile-submission-download" href="https://arxiv.org/html/2607.09153v1">HTML (experimental)</a>



    <blockquote class="abstract mathjax">
            <span class="descriptor">Abstract:</span>Process Reward Models (PRMs) have been proven to be highly effective in guiding test-time scaling (TTS) methods, which significantly boost the capabilities of LLM-based multi-agent systems. However, existing PRMs are text-based: they re-encode the entire trajectory text from scratch. In long multi-agent rollouts, the scoring cost, growing quadratically with respect to sequence length L, creates a severe computational bottleneck, severely limiting PRMs&#39; application in long-context scenarios. To resolve this, we introduce KV-PRM, a highly efficient process reward model that eliminates the heavy text re-encoding by directly reading the KV cache produced naturally during the LLM&#39;s generation phase. By processing a single &#34;verify token&#34; against the pre-existing KV cache, KV-PRM reduces the scoring cost from O(L^2) to O(L). We formally prove that the KV cache contains strictly greater information capacity than text, and is more efficient for downstream reward modeling. Empirically, across the MATH, GSM8K, and AIME benchmarks, KV-PRM matches or strictly outperforms text-PRMs under various TTS methods such as Beam Search, MCTS, and Weighted Voting, with up to a 5,000x reduction in scoring FLOPs, a 37x reduction in latency, and a 34x reduction in per-sequence memory footprint compared to text-based PRMs.
    </blockquote>
    ...
    <div class="metatable">
      <table summary="Additional metadata"><tr>
          ...
          <td class="tablecell arxivid"><span class="arxivid"><a href="https://arxiv.org/abs/2607.09153">arXiv:2607.09153</a> [cs.AI]</span></td>
        </tr>
        <tr>
          <td class="tablecell label">&nbsp;</td>
          <td class="tablecell arxividv">(or <span class="arxivid">
              <a href="https://arxiv.org/abs/2607.09153v1">arXiv:2607.09153v1</a> [cs.AI]</span> for this version)
          </td>
        </tr>
        <tr>
          <td class="tablecell label">&nbsp;</td>
          <td class="tablecell arxivdoi">              <a href="https://doi.org/10.48550/arXiv.2607.09153" id="arxiv-doi-link">https://doi.org/10.48550/arXiv.2607.09153</a><div class="button-and-tooltip">
    ...
    <div class="submission-history">
      <h2>Submission history</h2> From: Peng Kuang [<a href="/show-email/a46cdb76/2607.09153" rel="nofollow">view email</a>]      <br/>    <strong>[v1]</strong>
        Fri, 10 Jul 2026 07:16:43 UTC (212 KB)<br/>
</div>
  </div>
  <!--end leftcolumn-->
...
View a PDF of the paper titled KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling, by Peng Kuang and 7 other authors</div><li><a href="/pdf/2607.09153" aria-describedby="download-button-info" accesskey="f" class="abs-button download-pdf">View PDF</a></li><li><a href="https://arxiv.org/html/2607.09153v1" class="abs-button" id="latexml-download-link">HTML (experimental)</a></li><li><a href="/src/2607.09153" class="abs-button download-eprint">TeX Source
 </a></li></ul>
  ...
  <div class="prevnext">
<span class="arrow">
      <a class="abs-button prev-url" href="/prevnext?id=2607.09153&amp;function=prev&amp;context=cs.AI"
         accesskey="p" title="previous in cs.AI (accesskey p)" rel="nofollow">&lt;&nbsp;prev</a>
    </span>
    <span class="is-hidden-mobile">&nbsp; | &nbsp;</span>    <span class="arrow">
      <a class="abs-button next-url" href="/prevnext?id=2607.09153&amp;function=next&amp;context=cs.AI" accesskey="n"
    ...
    <div class="switch context-change">
        <a href="/abs/2607.09153?context=cs" rel="nofollow">cs</a><br class="is-hidden-mobile">
    </div>
  </div>
      ...
      <div class="extra-ref-cite">
        ...
        <ul>
          <li><a  class="abs-button abs-button-small cite-ads" href="https://ui.adsabs.harvard.edu/abs/arXiv:2607.09153">NASA ADS</a></li><li><a  class="abs-button abs-button-small cite-google-scholar" href="https://scholar.google.com/scholar_lookup?arxiv_id=2607.09153" target="_blank" rel="noopener">Google Scholar</a></li>
          <li><a  class="abs-button abs-button-small cite-semantic-scholar" href="https://api.semanticscholar.org/arXiv:2607.09153" target="_blank" rel="noopener">Semantic Scholar</a></li>
        </ul>
...
<div id='bib-cite-modal' class='bib-modal' hidden='true'>
  ...
  <div><h3>Bookmark</h3></div><a class="abs-button abs-button-grey abs-button-small" href="http://www.bibsonomy.org/BibtexHandler?requTask=upload&amp;url=https://arxiv.org/abs/2607.09153&amp;description=KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling"
     title="Bookmark on BibSonomy">
    <img src="/static/browse/0.3.4/images/icons/social/bibsonomy.png"
         alt="BibSonomy"/>
  </a>
  <a class="abs-button abs-button-grey abs-button-small" href="https://reddit.com/submit?url=https://arxiv.org/abs/2607.09153&amp;title=KV-PRM: Efficient Process Reward Modeling via KV-Cache Transfer for Multi-Agent Test-Time Scaling"
...
<!-- END LABS AREA -->
  <div class="endorsers">
    <a href="/auth/show-endorsers/2607.09153" class="endorser-who" rel="nofollow">Which authors of this paper are endorsers?</a> |
