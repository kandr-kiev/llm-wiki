...
class Balance:
    ...
    def apply_decision(cls, balance: Dict[str, float],
        ...
        # Pull-to-center: якщо дисбаланс > 25%, додатковий зсув до центру
    ...
    def is_enlightened(cls, balance: Dict[str, float],
        ...
        return all(abs(balance[k] - 25.0) <= tolerance for k in cls.QUALITIES)

    @classmethod
    def display(cls, balance: Dict[str, float]) -> str:
    ...
    def dominant_quality(cls, balance: Dict[str, float]) -> str:
        """Повертає домінуючу рису."""
        return max(balance, key=balance.get)

    @classmethod
    def shadow_quality(cls, balance: Dict[str, float]) -> str:
        """Повертає найменшу рису (тінь)."""
