...
def _get_phase_name(age: float) -> str:
    ...
    if age < 1.5:
        ...
        return "Ростючий серп"
    elif age < 8.5:
        return "Перша чверть"
    elif age < 13.5:
        return "Прибуваючий серп"
    elif age < 15.5:
...
def _get_illumination(age: float) -> float:
...
def _get_lunar_energy(lunar_day: int) -> str:
    """Get lunar day energy description from the traditional calendar table."""
    energies = {
...
def _get_lunar_advice(lunar_day: int) -> str:
    ...
    advices = {
        ...
        25: "Усамітніться. Послухайте внутрішній голос. Нікуди не поспішайте.",
        26: "Менше говоріть, більше слухайте. Економте гроші та сили. Не хвастайтеся.",
        27: "Шукайте відповіді на таємниці. Подорожуйте водою. Сплачуйте борги.",
        28: "Підбивайте підсумки місяця. Перебувайте в гарному настрої. Даруйте радість.",
        29: "Очищуйте тіло та житло. Підбивайте підсумки. Не плануйте майбутнє.",
        30: "Прощайте образи. Дякуйте за досвід. Завершуйте старий місячний цикл.",
    }
    return advices.get(lunar_day, "—")


def get_moon_phase(latitude: float = 50.4501, longitude: float = 30.5234, 
                   timezone: str = "Europe/Kiev") -> dict:
    """Calculate accurate moon phase using astral library.
    
    Args:
        latitude: Geographic latitude (default: Kyiv)
        longitude: Geographic longitude (default: Kyiv)
        timezone: IANA timezone name
    
    Returns dict with: age, phase, visibility, lunar_day, zodiac, symbol_of_day,
                     moonrise, moonset, moon_period
    """
    from astral.moon import phase as astral_phase, moonrise, moonset
    ...
    # Get moon age from astral (returns days 0-27.99)
    age = astral_phase(today_utc)
    
    # Calculate derived values
    phase_name = _get_phase_name(age)
    illumination = _get_illumination(age)
    lunar_day = _get_lunar_day(age)
    zodiac = _get_zodiac_sign(age)
    symbol = _get_lunar_symbol(lunar_day)
    energy = _get_lunar_energy(lunar_day)
    advice = _get_lunar_advice(lunar_day)
    
    # Get moonrise/moonset from astral
    loc = LocationInfo("Kyiv", "Ukraine", timezone, latitude, longitude)
    
    try:
        mr = moonrise(loc.observer, today_utc, tz)
        ms = moonset(loc.observer, today_utc, tz)
        moonrise_str = mr.strftime("%H:%M") if mr else "—"
        moonset_str = ms.strftime("%H:%M") if ms else "—"
    except Exception:
        moonrise_str = "—"
        moonset_str = "—"
    
    # Moon period (lunar day boundaries)
    next_day = now + __import__("datetime").timedelta(days=1)
    moon_period = f"{now.strftime('%d.%m.%y %H:%M')} — {next_day.strftime('%d.%m.%y %H:%M')}"
    
    # Format age as "Xd Xh Xm Xs"
    total_seconds = age * 24 * 3600
    days = int(total_seconds // 86400)
    hours = int((total_seconds % 86400) // 3600)
    minutes = int((total_seconds % 3600) // 60)
    seconds = int(total_seconds % 60)
    age_formatted = f"{days}д {hours}г:{minutes}х:{seconds}с"
    ...
    return {
        ...
        "lunar_day": lunar_day,
        "zodiac": zodiac,
        "symbol_of_day": symbol,
        "energy_of_day": energy,
        "advice_of_day": advice,
