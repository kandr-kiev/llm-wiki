...
def parse_rss(content):
    ...
    for match in matches:
        ...
        link_m = re.search(r"<link>(.*?)</link>", match, re.DOTALL)
        date_m = re.search(r"<pubDate>(.*?)</pubDate>", match, re.DOTALL)
        desc_m = re.search(r"<description>(.*?)</description>", match, re.DOTALL)
        item = {}
        if title_m:
