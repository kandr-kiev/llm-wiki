#!/usr/bin/env python3
"""Fix broken wikilinks with proper fuzzy matching."""
import re
...
def get_stem_base(stem):
    """Remove date and version suffixes: _2026-07-07, _1, _2, etc."""
...
# Word suffix map: try removing first words
word_map = {}
for norm, stem in direct_map.items():
    parts = norm.split('-')
...
# Prefix map: try removing last words (for long wikilinks)
prefix_map = {}
for norm, stem in direct_map.items():
    parts = norm.split('-')
...
def find_best_match(wl):
    ...
    # 4. Strip last words (prefix match)
    for i in range(1, len(parts) - 1):
        prefix = '-'.join(parts[:i])
